# Accessible Registration Form UI

A fully accessible registration form built with React, TypeScript, and Tailwind CSS, following WCAG AA accessibility guidelines.

## 🎯 Features

- ✅ **WCAG AA Compliant** - Meets accessibility standards
- ✅ **Keyboard Navigation** - Full keyboard support
- ✅ **Screen Reader Friendly** - Proper ARIA labels and live regions
- ✅ **Password Strength Indicator** - Real-time validation feedback
- ✅ **Progress Tracking** - Visual form completion percentage
- ✅ **Responsive Design** - Works on all devices
- ✅ **Password Visibility Toggle** - Show/hide password feature
- ✅ **Modern Tech Stack** - Built with React and TypeScript

## 🛠️ Technologies Used

- **React** - UI component library
- **TypeScript** - Type-safe JavaScript
- **Tailwind CSS** - Utility-first CSS framework
- **Vite** - Fast build tool and dev server
- **HTML5** - Semantic markup
- **ARIA** - Accessibility attributes

## ♿ Accessibility Features

- Proper label associations with `for` attribute
- ARIA live regions for dynamic content announcements
- Focus states for keyboard navigation
- Color contrast meeting WCAG standards
- Required field indicators with proper ARIA
- Error message associations
- Password toggle with state announcements
- Logical tab order through all form elements

## 🚀 Live Demo

__[View Live Demo](____https://jeevanm2004.github.io/accessible-form-ui____)__

## 📦 Installation

1. Clone the repository:
```bash
git clone https://github.com/Jeevanm2004/accessible-form-ui.git
cd accessible-form-ui
```

2. Install dependencies:
```bash
npm install
```

3. Run the development server:
```bash
npm run dev
```

4. Open your browser and navigate to `http://localhost:5173`

## 🏗️ Build for Production

```bash
npm run build
```

The production-ready files will be in the `dist` folder.

## 🧪 Testing

Use Control + Shift + m to toggle the tab key moving focus. Alternatively, use esc then tab to move to the next interactive element on the page.

## 📝 Acknowledgments

Built as part of the **roadmap.sh** frontend development curriculum.
